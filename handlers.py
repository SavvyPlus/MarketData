...
def process_MDF_300(conn,version_header,toks,last_rec_ind,last100,last200,source_file_id):
    ...
    thisid = curs.fetchone()[0]
    curs.execute('INSERT MDF_Interval_Key (FileDetailsID,StreamDetailsID,ReadingID,source_file_id) OUTPUT Inserted.ID VALUES (?,?,?,?)',(last100[-1],last200[-1],thisid,source_file_id))
    ...
    if toks[qm][0] != 'V':        
        (tf,res) = process_MDF_400(conn,version_header,['1',str(n_readings),toks[qm],toks[qm+1],toks[qm+2]],'300',last200,vals,None)
        if not tf:
            return (False,[])
    ...
    with open(fname, 'rt') as f:
        ...
        while last_rec_ind != '900':
            ...
            if rec_ind == '100':
                ...
                status,last400 = process_MDF_400(conn,version_header,toks[1:],last_rec_ind,last200,last300,last400)
            elif rec_ind == '500':
